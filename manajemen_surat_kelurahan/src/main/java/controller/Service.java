...
public class Service {
    ...
    public void cariSurat(String nomorSurat) {
        for (Surat s : daftarSurat) {
            if (s.getNomorSurat().equalsIgnoreCase(nomorSurat)) {
                s.tampilkanInfo();
                return;
            }
        }
        System.out.println("Surat dengan nomor " + nomorSurat + " tidak ditemukan.");
    }
    public void cariSurat(String judulSurat, boolean byJudul) {
        for (Surat s : daftarSurat) {
            if (s.getJudulSurat().equalsIgnoreCase(judulSurat)) {
                s.tampilkanInfo();
                return;
            }
        }
        System.out.println("Surat dengan judul \"" + judulSurat + "\" tidak ditemukan.");
    }

    // Untuk input
    public void cariSuratByNomorInput() {
        ...
        cariSurat(nomor); // panggil versi 1
    }

    public void cariSuratByJudulInput() {
        System.out.print("Masukkan Judul Surat yang dicari: ");
        String judul = input.nextLine();
        if (judul.isEmpty()){
            System.out.println("Input tidak boleh kosong! Silahkan coba lagi");
            return;
        }
        cariSurat(judul, true); // panggil versi 2
    }
}
